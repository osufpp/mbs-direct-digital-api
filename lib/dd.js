...
DirectDigital.prototype._http = function (options) {
    ...
    var urlParams = querystring.stringify(options.qs); // request module not handling arrayed url parameters in the way they expect
    options.url = options.url + '?' + urlParams; // hand-building URL parameters
    delete options.qs;
 ...
 * @private
 */
DirectDigital.prototype._post = function (endpoint, servicesPath, qs) {
    var options = {
        method: 'POST',
        url: this._buildApiUrl(endpoint, servicesPath),
        qs: qs
